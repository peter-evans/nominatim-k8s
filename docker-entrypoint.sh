...
postgres_datadir() {
  ...
  sed -i "s:data_directory.*:data_directory = '${NOMINATIM_POSTGRESQL_DATA_PATH}':" /etc/postgresql/*/main/postgresql.conf
  if [ ! -f ${NOMINATIM_POSTGRESQL_DATA_PATH}/PG_VERSION ]
  then
    sudo -u postgres /usr/lib/postgresql/*/bin/initdb -D ${NOMINATIM_POSTGRESQL_DATA_PATH}
  fi
}
